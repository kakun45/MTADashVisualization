...
app.layout = html.Div([
    ...
    html.Iframe(id='map', srcDoc=open('Map4.html', 'r').read(), width = '100%', height='300'),
    dcc.Graph(id='graph2'),
    dcc.Graph(id='graph'),
...
def update_figure_map(selected_time):
    ...
    for lt, ln, ex, st in zip(lat, lon, exits, station):
        fgv.add_child(folium.CircleMarker(location=[lt, ln], radius = ex/500, popup=str(ex)+' ppl Exit @ '+(st), 
        fill_color=color_producer_ex(ex), fill=False, color='red', fill_opacity=0.1))
    
    fgp = folium.FeatureGroup(name='MTA_Entries')
    for lt, ln, en, st in zip(lat, lon, entries, station):
        fgp.add_child(folium.CircleMarker(location=[lt, ln], radius = en/500, popup=str(en)+' ppl Enter @'+(st), 
        fill_color=color_producer_ent(en), fill=False, color='green', fill_opacity=0.2))
