...
def map59th():
    map = folium.Map(
        location=[40.7628, -73.9676], zoom_start=70, tiles="Stamen Terrain"
    )

    folium.CircleMarker(
        location=[40.7628, -73.9676],
...
#     folium.CircleMarker(
#         [r for r in df[station_id]: location=[df['LAT'], df['LON']],
#         popup = df['station_id'],
...
def grouping_sum2(df, group_col, sum_col, new_col):
    df2 = df.copy()
