...
for lt, ln, ex, st in zip(lat, lon, exits, station):
    fgv.add_child(folium.CircleMarker(location=[lt, ln], radius = ex/10000, popup=str(ex)+' ppl Exit @ '+(st), 
...
for lt, ln, en, st in zip(lat, lon, entries, station):
    fgp.add_child(folium.CircleMarker(location=[lt, ln], radius = en/10000, popup=str(en)+' ppl Enter @'+(st), 
    fill_color=color_producer_ent(en), fill=False, color='green', fill_opacity=0.1))
